...
module CrudHelper
  ...
  def crud_links path_array, options={}
    ...
    content_tag :ul, class: 'buttons' do
       ...
       concat content_tag(:li, link_to("Neu", [:new] + path_struct + [model.model_name.underscore], class: 'new', title: 'Löschen')) if !options[:except].include? :new && !record.try(:new_record?) && can?(:create, record)
    end
  end
