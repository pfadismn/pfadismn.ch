    ...
    if option.nodeName is "OPTION"
      if option.text != ""
        ...
        @parsed.push
          ...
          options_index: @options_index
          #empty: true
          # HACK (or feature!?) BEGIN XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
          value: ""
          text: ""
          html: "&nbsp;"
          selected: option.selected
          disabled: if group_disabled is true then group_disabled else option.disabled
          group_array_index: group_position
          # END XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
      @options_index += 1
...
class Chosen
  ...
  constructor: (elmn) ->
    ...
    text = $("#js_translations_please_choose").text() || "Select an Option"
    text_multiple = $("#js_translations_please_choose_multiple").text() || "Select Some Options"
    @default_text_default = if @form_field.multiple then text_multiple else text
  ...
  no_results: (terms) ->
    text = $("#js_translations_no_results_match").text() || "No results match"
    no_results_html = $('<li class="no-results">' + text + ' "<span></span>"</li>')
